...
BETankPlayerView::BETankPlayerView(BEBlock *block) : BEBlockView(block), m_towardNode(nullptr), m_gridIdNode(nullptr)
{
	BETank *tank = dynamic_cast<BETank *>(getBlock());
	tank->addView(this);
}

BETankPlayerView::~BETankPlayerView()
{
	BETank *tank = dynamic_cast<BETank *>(getBlock());
...
bool BETankPlayerView::init()
{
	const CCSize &size = getContentSize();
	CCLayerColor *layerColor = CCLayerColor::create(ccc4(0, 255, 10, 255));
...
void BETankPlayerView::onTankPositionChanged(const GeoPoint &pos)
{
...
void BETankPlayerView::onTankTowardChanged(int toward)
{
	const CCSize &size = getContentSize();
	BETank *tank = dynamic_cast<BETank *>(getBlock());
	changeToward(m_towardNode, tank->getToward(), size);
...
BETankPlayerView *BETankPlayerView::create(BEBlock *block)
{
	BETankPlayerView *grassBlock = new BETankPlayerView(block);
